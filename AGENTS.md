...
- Prefer using Nia (CLI or MCP) for upstream package documentation, breaking-change research, and dependency-upgrade planning when it is enabled and relevant.
...
- When implementing from a user-attached plan file, do not edit that plan file; apply changes in the codebase and use todos for tracking.
...
- Production is deployed on Render at https://cursor-event-portal.onrender.com; example service names in `render.yaml` may not match the live service name shown in the Render dashboard. If the live site shows stale branding or outdated copy, confirm a recent successful deploy from `main` and that the URL matches the active service (older bookmarks may still use a legacy Render hostname).
